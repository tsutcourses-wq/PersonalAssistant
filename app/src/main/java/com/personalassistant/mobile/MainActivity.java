...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "3.8.2 Android";
    ...
    // Dashboard calendar navigation state. The database remains unchanged; this only controls the view.
    private String dashboardCalendarMode = "week";
    private int dashboardCalendarYear = 0;
    private int dashboardCalendarMonth = 0;
    private int dashboardCalendarDay = 0;
    ...
    private void addEqualRowButton(LinearLayout parent, Button button, View.OnClickListener listener) {
        button.setOnClickListener(listener);
        button.setTextSize(12);
        button.setPadding(dp(5), dp(6), dp(5), dp(6));
    ...
    private void showSideMenu() {
        ...
        String[] names = {"Dashboard", "Reminders", "All tasks", "Settings"};
        Runnable[] actions = {
                this::showDashboard,
                this::showReminders,
                this::showAllTasks,
    ...
    private void showDashboard() {
        ...
        content.addView(buildTodayDateCard());
        content.addView(buildDashboardCalendar());

        LinearLayout upcoming = card();
        upcoming.addView(tv("Upcoming reminders", 18, textColor, Typeface.BOLD));
        Cursor cursor = db.getReadableDatabase().rawQuery(
                "SELECT id,title,jdate,time_text,details FROM reminders WHERE remind_at_iso>=? ORDER BY remind_at_iso LIMIT 5",
        ...
        tasks.addView(tv("Upcoming tasks", 18, textColor, Typeface.BOLD));
        int shown = addDashboardTasks(tasks);
        if (shown == 0) tasks.addView(tv("No open tasks.", 13, mutedColor, Typeface.NORMAL));
        addButton(tasks, btn("+ Add task"), v -> editTask(0));
        addButton(tasks, outlineBtn("See all tasks"), v -> showAllTasks());
        content.addView(tasks);

        LinearLayout notes = card();
        notes.addView(tv("Notes", 18, textColor, Typeface.BOLD));
        final EditText noteTitle = input("Note title", "", false);
        final EditText noteBody = input("Note", "", true);
        noteBody.setMinLines(5);
        notes.addView(noteTitle);
        notes.addView(noteBody);
        addButton(notes, btn("Save note"), v -> {
            String titleValue = noteTitle.getText().toString().trim();
            String bodyValue = noteBody.getText().toString().trim();
            if (titleValue.isEmpty()) { toast("Enter note title."); return; }
            if (bodyValue.isEmpty()) { toast("Enter note text."); return; }
            ContentValues cv = new ContentValues();
            cv.put("title", titleValue);
            cv.put("body", bodyValue);
            cv.put("created_at", nowIso());
            cv.put("updated_at", nowIso());
            insert("dashboard_notes", cv);
            noteTitle.setText("");
            noteBody.setText("");
            toast("Note saved.");
        });
        addButton(notes, outlineBtn("See all notes"), v -> showAllNotes());
        content.addView(notes);

        setScreen("Dashboard", content, this::showDashboard, "dashboard");
    }

    private LinearLayout buildTodayDateCard() {
        LinearLayout todayCard = card();
        todayCard.addView(tv("Today", 18, textColor, Typeface.BOLD));
        todayCard.addView(tv("Shamsi: " + Jalali.today(), 15, primaryColor, Typeface.BOLD));
        String gregorian = new SimpleDateFormat("yyyy/MM/dd", Locale.US).format(new Date());
        todayCard.addView(tv("Gregorian: " + gregorian, 15, textColor, Typeface.NORMAL));
        return todayCard;
    }

    private void ensureDashboardCalendarAnchor() {
        if (dashboardCalendarYear > 0) return;
        String[] parts = Jalali.today().split("/");
        try {
            dashboardCalendarYear = Integer.parseInt(parts[0]);
            dashboardCalendarMonth = Integer.parseInt(parts[1]);
            dashboardCalendarDay = Integer.parseInt(parts[2]);
        } catch (Exception ex) {
            dashboardCalendarYear = Jalali.currentYear();
            dashboardCalendarMonth = 1;
            dashboardCalendarDay = 1;
        }
    }

    private void resetDashboardCalendarToToday() {
        dashboardCalendarYear = 0;
        dashboardCalendarMonth = 0;
        dashboardCalendarDay = 0;
        ensureDashboardCalendarAnchor();
        showDashboard();
    }

    private void setDashboardCalendarMode(String mode) {
        dashboardCalendarMode = mode;
        showDashboard();
    }

    private void shiftDashboardCalendar(int direction) {
        ensureDashboardCalendarAnchor();
        if ("week".equals(dashboardCalendarMode)) {
            String anchor = String.format(Locale.US, "%04d/%02d/%02d", dashboardCalendarYear, dashboardCalendarMonth, dashboardCalendarDay);
            long millis = Jalali.toMillis(anchor, "12:00") + direction * 7L * 86400000L;
            String[] p = Jalali.fromMillis(millis).split("/");
            dashboardCalendarYear = Integer.parseInt(p[0]);
            dashboardCalendarMonth = Integer.parseInt(p[1]);
            dashboardCalendarDay = Integer.parseInt(p[2]);
        } else if ("month".equals(dashboardCalendarMode)) {
            dashboardCalendarMonth += direction;
            if (dashboardCalendarMonth < 1) { dashboardCalendarMonth = 12; dashboardCalendarYear--; }
            if (dashboardCalendarMonth > 12) { dashboardCalendarMonth = 1; dashboardCalendarYear++; }
            dashboardCalendarDay = Math.min(dashboardCalendarDay, Jalali.daysInMonth(dashboardCalendarYear, dashboardCalendarMonth));
        } else {
            dashboardCalendarYear += direction;
            dashboardCalendarDay = Math.min(dashboardCalendarDay, Jalali.daysInMonth(dashboardCalendarYear, dashboardCalendarMonth));
        }
        showDashboard();
    }

    private LinearLayout buildDashboardCalendar() {
        ensureDashboardCalendarAnchor();
        LinearLayout box = card();
        String modeTitle = "week".equals(dashboardCalendarMode) ? "Week" : ("month".equals(dashboardCalendarMode) ? "Month" : "Year");
        box.addView(tv("Calendar • " + modeTitle, 18, textColor, Typeface.BOLD));
        addButton(box, btn("+ Add reminder"), v -> editReminder(0));

        LinearLayout modes = hbox();
        addEqualRowButton(modes, outlineBtn("Week"), v -> setDashboardCalendarMode("week"));
        addEqualRowButton(modes, outlineBtn("Month"), v -> setDashboardCalendarMode("month"));
        addEqualRowButton(modes, outlineBtn("Year"), v -> setDashboardCalendarMode("year"));
        box.addView(modes);

        LinearLayout nav = hbox();
        addEqualRowButton(nav, outlineBtn("‹ Previous"), v -> shiftDashboardCalendar(-1));
        addEqualRowButton(nav, outlineBtn("Next ›"), v -> shiftDashboardCalendar(1));
        box.addView(nav);

        if ("month".equals(dashboardCalendarMode)) addDashboardMonthView(box);
        else if ("year".equals(dashboardCalendarMode)) addDashboardYearView(box);
        else addDashboardWeekView(box);
        return box;
    }

    private void addDashboardWeekView(LinearLayout box) {
        String anchor = String.format(Locale.US, "%04d/%02d/%02d", dashboardCalendarYear, dashboardCalendarMonth, dashboardCalendarDay);
        Calendar start = Calendar.getInstance();
        start.setTimeInMillis(Jalali.toMillis(anchor, "12:00"));
        int offset = (start.get(Calendar.DAY_OF_WEEK) - Calendar.SATURDAY + 7) % 7;
        start.add(Calendar.DAY_OF_MONTH, -offset);

        // Keep the whole week visible without horizontal scrolling, but use two
        // rows so each day card remains readable on a phone screen. The first
        // row contains four days and the second row contains three days plus a
        // blank spacer, preserving the same card width in both rows.
        String[] shortNames = {"ش", "ی", "د", "س", "چ", "پ", "ج"};
        for (int row = 0; row < 2; row++) {
            LinearLayout weekRow = hbox();
            weekRow.setPadding(0, row == 0 ? dp(8) : dp(4), 0, dp(4));
            for (int col = 0; col < 4; col++) {
                ...
                if (i < 7) {
                    ...
                    String dayLabel = String.format(Locale.US, "%02d/%02d", j[1], j[2]);
                    LinearLayout dayCard = buildDashboardDayCard(date, shortNames[i], dayLabel);
                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, dp(76), 1);
                    lp.setMargins(dp(2), 0, dp(2), 0);
                    weekRow.addView(dayCard, lp);
                } else {
                    View spacer = new View(this);
                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, dp(76), 1);
                    lp.setMargins(dp(2), 0, dp(2), 0);
                    weekRow.addView(spacer, lp);
                }
            }
            box.addView(weekRow);
        }
    ...
    private void addDashboardMonthView(LinearLayout box) {
        String[] monthNames = {"فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند"};
        box.addView(tv(monthNames[dashboardCalendarMonth - 1] + " " + dashboardCalendarYear, 15, primaryColor, Typeface.BOLD));
        LinearLayout weekdays = hbox();
        String[] shortNames = {"ش", "ی", "د", "س", "چ", "پ", "ج"};
        for (String name : shortNames) {
            TextView label = tv(name, 12, mutedColor, Typeface.BOLD);
            label.setGravity(Gravity.CENTER);
            weekdays.addView(label, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1));
        }
        box.addView(weekdays);

        int[] g = Jalali.jalaliToGregorian(dashboardCalendarYear, dashboardCalendarMonth, 1);
        Calendar first = Calendar.getInstance();
        first.clear();
        first.set(g[0], g[1] - 1, g[2], 12, 0, 0);
        int leading = (first.get(Calendar.DAY_OF_WEEK) - Calendar.SATURDAY + 7) % 7;
        int days = Jalali.daysInMonth(dashboardCalendarYear, dashboardCalendarMonth);
        int dayNo = 1;
        for (int row = 0; row < 6 && dayNo <= days; row++) {
            LinearLayout line = hbox();
            for (int col = 0; col < 7; col++) {
                if ((row == 0 && col < leading) || dayNo > days) {
                    TextView blank = tv("", 12, mutedColor, Typeface.NORMAL);
                    line.addView(blank, new LinearLayout.LayoutParams(0, dp(64), 1));
                } else {
                    String date = String.format(Locale.US, "%04d/%02d/%02d", dashboardCalendarYear, dashboardCalendarMonth, dayNo);
                    LinearLayout dayCard = buildDashboardDayCard(date, "", String.valueOf(dayNo));
                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, dp(64), 1);
                    lp.setMargins(dp(2), dp(2), dp(2), dp(2));
                    line.addView(dayCard, lp);
                    dayNo++;
                }
            }
            box.addView(line);
    ...
    private void addDashboardYearView(LinearLayout box) {
        String[] monthNames = {"فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند"};
        box.addView(tv("Year " + dashboardCalendarYear, 15, primaryColor, Typeface.BOLD));
        for (int row = 0; row < 4; row++) {
            LinearLayout line = hbox();
            for (int col = 0; col < 3; col++) {
                final int month = row * 3 + col + 1;
                int count = countRemindersForMonth(dashboardCalendarYear, month);
                Button monthButton = outlineBtn(month + " • " + monthNames[month - 1] + (count > 0 ? "\n" + count + " reminder" + (count == 1 ? "" : "s") : ""));
                monthButton.setAllCaps(false);
                monthButton.setOnClickListener(v -> {
                    dashboardCalendarMonth = month;
                    dashboardCalendarDay = 1;
                    dashboardCalendarMode = "month";
                    showDashboard();
                });
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, dp(78), 1);
                lp.setMargins(dp(3), dp(3), dp(3), dp(3));
                line.addView(monthButton, lp);
            }
            box.addView(line);
        }
    }

    private LinearLayout buildDashboardDayCard(String date, String dayName, String dayLabel) {
        int reminderCount = countRemindersForDate(date);
        boolean today = date.equals(Jalali.today());
        LinearLayout dayCard = vbox();
        dayCard.setGravity(Gravity.CENTER);
        dayCard.setPadding(dp(4), dp(5), dp(4), dp(5));
        dayCard.setBackground(rounded(today ? primaryColor : panel2Color, 10));
        if (!dayName.isEmpty()) dayCard.addView(tv(dayName, 12, today ? 0xFFFFFFFF : textColor, Typeface.BOLD));
        dayCard.addView(tv(dayLabel, 14, today ? 0xFFFFFFFF : primaryColor, Typeface.BOLD));
        if (reminderCount > 0) dayCard.addView(tv(String.valueOf(reminderCount), 10, today ? 0xFFFFFFFF : accentColor, Typeface.BOLD));
        dayCard.setOnClickListener(v -> showRemindersForDate(date));
        return dayCard;
    }
    ...
    private int countRemindersForMonth(int year, int month) {
        String prefix = String.format(Locale.US, "%04d/%02d/%%", year, month);
        Cursor cursor = db.getReadableDatabase().rawQuery("SELECT COUNT(*) FROM reminders WHERE jdate LIKE ?", new String[]{prefix});
        try { return cursor.moveToFirst() ? cursor.getInt(0) : 0; }
        finally { cursor.close(); }
    }
    ...
    private int addDashboardTasks(LinearLayout parent) {
        int shown = 0;
        Cursor tasks = db.getReadableDatabase().rawQuery(
                "SELECT id,title,due_jdate,responsible FROM todos WHERE done=0 " +
                        "ORDER BY CASE WHEN due_jdate IS NULL OR due_jdate='' THEN 1 ELSE 0 END, due_jdate ASC, created_at DESC LIMIT 5",
                null
        );
        try {
            while (tasks.moveToNext()) {
                final long id = tasks.getLong(0);
                LinearLayout item = vbox();
                item.setPadding(dp(10), dp(8), dp(10), dp(8));
                item.setBackground(rounded(panel2Color, 10));
                item.addView(tv("• " + tasks.getString(1), 14, textColor, Typeface.BOLD));
                String due = tasks.getString(2) == null ? "" : tasks.getString(2);
                String responsible = tasks.getString(3) == null ? "" : tasks.getString(3);
                String meta = (due.isEmpty() ? "No due date" : "Due: " + due) + (responsible.isEmpty() ? "" : " • " + responsible);
                item.addView(tv(meta, 12, mutedColor, Typeface.NORMAL));
                item.setOnClickListener(v -> editTask(id));
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                lp.setMargins(0, dp(4), 0, dp(4));
    ...
    private void showAllNotes() {
        ...
        top.addView(tv("All notes", 18, textColor, Typeface.BOLD));
        addButton(top, btn("+ Add note"), v -> editNote(0));
        content.addView(top);

        Cursor notes = db.getReadableDatabase().rawQuery(
                "SELECT id,title,body,created_at,updated_at FROM dashboard_notes ORDER BY updated_at DESC, id DESC",
                null
        );
        try {
            if (!notes.moveToFirst()) {
                LinearLayout empty = card();
                empty.addView(tv("No saved notes yet.", 14, mutedColor, Typeface.NORMAL));
                content.addView(empty);
            } else {
                do {
                    final long noteId = notes.getLong(0);
                    LinearLayout item = card();
                    item.addView(tv(notes.getString(1), 16, textColor, Typeface.BOLD));
                    String body = notes.getString(2) == null ? "" : notes.getString(2);
                    if (!body.isEmpty()) item.addView(tv(body, 13, textColor, Typeface.NORMAL));
                    String updated = notes.getString(4) == null ? "" : notes.getString(4);
                    if (!updated.isEmpty()) item.addView(tv("Updated: " + updated.replace('T', ' '), 11, mutedColor, Typeface.NORMAL));
                    addButton(item, outlineBtn("Edit"), v -> editNote(noteId));
                    addButton(item, dangerBtn("Delete"), v -> confirm("Delete note", "Delete this note?", () -> {
                        delete("dashboard_notes", noteId);
                        showAllNotes();
                    }));
        ...
        setScreen("All notes", content, this::showAllNotes, "notes");
    }
    ...
    private void editNote(long id) {
        ...
        String titleValue = "";
        String bodyValue = "";
        String createdValue = nowIso();
        if (exists) {
            Cursor cursor = db.getReadableDatabase().rawQuery(
                    "SELECT title,body,created_at FROM dashboard_notes WHERE id=?",
                    new String[]{String.valueOf(id)}
            );
            try {
                if (cursor.moveToFirst()) {
                    titleValue = cursor.getString(0) == null ? "" : cursor.getString(0);
                    bodyValue = cursor.getString(1) == null ? "" : cursor.getString(1);
                    createdValue = cursor.getString(2) == null ? nowIso() : cursor.getString(2);
                }
            } finally { cursor.close(); }
        }

        final String originalCreated = createdValue;
        LinearLayout content = vbox();
        LinearLayout form = card();
        form.addView(tv(exists ? "Edit note" : "Add note", 18, textColor, Typeface.BOLD));
        final EditText title = input("Note title", titleValue, false);
        final EditText body = input("Note", bodyValue, true);
        body.setMinLines(6);
        form.addView(title);
        form.addView(body);
        addButton(form, btn(exists ? "Save changes" : "Save note"), v -> {
            String titleText = title.getText().toString().trim();
            String bodyText = body.getText().toString().trim();
            if (titleText.isEmpty()) { toast("Enter note title."); return; }
            if (bodyText.isEmpty()) { toast("Enter note text."); return; }
            ContentValues cv = new ContentValues();
            cv.put("title", titleText);
            cv.put("body", bodyText);
            cv.put("created_at", originalCreated);
            cv.put("updated_at", nowIso());
            if (exists) {
                update("dashboard_notes", cv, id);
                toast("Note updated.");
            } else {
                insert("dashboard_notes", cv);
                toast("Note saved.");
        ...
        if (exists) {
            addButton(form, dangerBtn("Delete note"), v -> confirm("Delete note", "Delete this note?", () -> {
                delete("dashboard_notes", id);
                skipHistoryOnce = true;
                showAllNotes();
            }));
        }
        ...
        setScreen(exists ? "Edit note" : "Add note", content, () -> editNote(id), "note:" + id);
    }
    ...
    private CentralServerClient.Settings loadCentralServerSettings() {
        SharedPreferences prefs = getSharedPreferences("pa_server", MODE_PRIVATE);
        CentralServerClient.Settings settings = new CentralServerClient.Settings();
        settings.url = prefs.getString("url", "");
        settings.username = prefs.getString("username", "");
        settings.password = prefs.getString("password", "");
        settings.revision = prefs.getInt("revision", 0);
        return settings;
    }

    private void saveCentralServerSettings(CentralServerClient.Settings settings) {
        getSharedPreferences("pa_server", MODE_PRIVATE).edit()
                .putString("url", settings.url == null ? "" : settings.url.trim())
                .putString("username", settings.username == null ? "" : settings.username)
                .putString("password", settings.password == null ? "" : settings.password)
                .putInt("revision", settings.revision)
                .apply();
    }

    private void showCentralServerConfigDialog() {
        CentralServerClient.Settings saved = loadCentralServerSettings();
        LinearLayout form = vbox();
        form.setPadding(dp(18), dp(8), dp(18), 0);
        EditText url = input("Server address", saved.url, false);
        EditText username = input("Username", saved.username, false);
        EditText password = input("Password", saved.password, false);
        ...
        alertBuilder()
                ...
                .setMessage("For local testing use an address such as http://192.168.1.10:8000. Use HTTPS before internet deployment.")
                .setView(form)
                .setPositiveButton("Save", (dialog, which) -> {
                    saved.url = url.getText().toString().trim();
                    saved.username = username.getText().toString();
                    saved.password = password.getText().toString();
                    saveCentralServerSettings(saved);
                    showSettings();
                })
                .setNegativeButton("Cancel", null)
                .show();
    ...
    private AlertDialog showServerWorking(String message) {
    ...
    private void testCentralServer() {
        CentralServerClient.Settings settings = loadCentralServerSettings();
        AlertDialog working = showServerWorking("Testing connection…");
        new Thread(() -> {
            try {
                CentralServerClient client = new CentralServerClient(settings);
                JSONObject health = client.health();
                client.login();
                JSONObject info = client.snapshotInfo();
                settings.revision = info.optInt("revision", settings.revision);
                saveCentralServerSettings(settings);
                runOnUiThread(() -> {
                    working.dismiss();
                    alertBuilder().setTitle("Central server")
                            .setMessage("Connection successful.\nStatus: " + health.optString("status", "ok") + "\nServer revision: " + info.optInt("revision", 0))
                ...
                runOnUiThread(() -> {
                    working.dismiss();
                    alertBuilder().setTitle("Central server error").setMessage(ex.getMessage()).setPositiveButton("OK", null).show();
                });
            }
        }).start();
    }

    private void uploadCentralSnapshot(boolean force) {
        CentralServerClient.Settings settings = loadCentralServerSettings();
        AlertDialog working = showServerWorking(force ? "Force-uploading all information and files…" : "Preparing and uploading all information and files…");
        new Thread(() -> {
            try {
                byte[] snapshot = SnapshotManager.createSnapshot(this, db, APP_VERSION);
                CentralServerClient client = new CentralServerClient(settings);
                JSONObject result = client.uploadSnapshot(snapshot, settings.revision, force);
                settings.revision = result.optInt("revision", settings.revision);
                saveCentralServerSettings(settings);
                runOnUiThread(() -> {
                    working.dismiss();
                    alertBuilder().setTitle("Central server")
                            .setMessage("Upload completed. Server revision: " + settings.revision)
                            .setPositiveButton("OK", null).show();
                    showSettings();
                });
            } catch (CentralServerClient.ConflictException conflict) {
                runOnUiThread(() -> {
                    working.dismiss();
                    alertBuilder().setTitle("Server has newer data")
                            .setMessage("The server is at revision " + conflict.currentRevision + " while this phone last downloaded revision " + settings.revision + ". Download first is recommended. Force-replace the server with this phone anyway?")
                            .setPositiveButton("Force upload", (dialog, which) -> uploadCentralSnapshot(true))
    ...
    private void confirmDownloadCentralSnapshot() {
        alertBuilder().setTitle("Download server data")
                .setMessage("This creates a local backup, then replaces the phone database and managed files with the server version. Continue?")
                .setPositiveButton("Download", (dialog, which) -> downloadCentralSnapshot())
                .setNegativeButton("Cancel", null)
                .show();
    }

    private void downloadCentralSnapshot() {
        CentralServerClient.Settings settings = loadCentralServerSettings();
        AlertDialog working = showServerWorking("Downloading all information and files…");
        new Thread(() -> {
            try {
                CentralServerClient.DownloadResult result = new CentralServerClient(settings).downloadSnapshot();
                try { db.close(); } catch (Exception ignored) { }
                SnapshotManager.applySnapshot(this, result.data);
                ...
                runOnUiThread(() -> {
                    ...
                    working.dismiss();
                    alertBuilder().setTitle("Central server")
                            .setMessage("Download completed. Local backup created. Revision: " + settings.revision)
                            .setPositiveButton("OK", null).show();
                    showSettings();
                });
            } catch (Exception ex) {
                runOnUiThread(() -> {
                    ...
                    try { db.getWritableDatabase(); } catch (Exception ignored) { }
                    working.dismiss();
                    alertBuilder().setTitle("Central server error").setMessage(ex.getMessage()).setPositiveButton("OK", null).show();
                });
            }
    ...
    private void editTask(long id) {
        ...
        if (exists) {
            Cursor cursor = db.getReadableDatabase().rawQuery(
                    "SELECT title,responsible,due_jdate,details,done FROM todos WHERE id=?",
                    new String[]{String.valueOf(id)}
            );
            try {
                if (cursor.moveToFirst()) {
                    titleValue = cursor.getString(0) == null ? "" : cursor.getString(0);
                    responsibleValue = cursor.getString(1) == null ? "" : cursor.getString(1);
                    dueValue = cursor.getString(2) == null ? "" : cursor.getString(2);
                    detailsValue = cursor.getString(3) == null ? "" : cursor.getString(3);
                    doneValue = cursor.getInt(4);
                }
            } finally { cursor.close(); }
        }
        ...
        LinearLayout content = vbox();
        LinearLayout form = card();
        form.addView(tv(exists ? "Edit task" : "Add task", 18, textColor, Typeface.BOLD));
        final EditText title = input("Task title", titleValue, false);
        final EditText responsible = input("Responsible", responsibleValue, false);
        form.addView(title);
        form.addView(responsible);
        final EditText due = dateField(form, "Due date", dueValue, false);
        final EditText details = input("Details", detailsValue, true);
        details.setMinLines(3);
        form.addView(details);
        final CheckBox done = new CheckBox(this);
        done.setText("Completed");
        done.setTextColor(textColor);
        done.setChecked(doneValue == 1);
        form.addView(done);

        addButton(form, btn(exists ? "Save changes" : "Save task"), v -> {
            String value = title.getText().toString().trim();
            if (value.isEmpty()) { toast("Enter task title."); return; }
            ContentValues cv = new ContentValues();
            cv.put("title", value);
            cv.put("due_iso", "");
            ...
            cv.put("details", details.getText().toString());
            cv.put("done", done.isChecked() ? 1 : 0);
            if (exists) {
                update("todos", cv, id);
                toast("Task updated.");
            } else {
        ...
        if (exists) {
            addButton(form, outlineBtn("Delete task"), v -> confirm("Delete task", "Delete this task?", () -> {
                delete("todos", id);
                skipHistoryOnce = true;
                showAllTasks();
            }));
        ...
        addButton(form, outlineBtn("Back"), v -> goBackOneStep());
        content.addView(form);
        setScreen(exists ? "Edit task" : "Add task", content, () -> editTask(id), "task:" + id);
    }
    ...
    private void showAllTasks() {
        ...
        top.addView(tv("Tasks are independent and are not tied to projects or students.", 13, mutedColor, Typeface.NORMAL));
        addButton(top, btn("+ Add task"), v -> editTask(0));
        Cursor completedCountCursor = db.getReadableDatabase().rawQuery("SELECT COUNT(*) FROM todos WHERE done=1", null);
        int completedCount = 0;
        try { if (completedCountCursor.moveToFirst()) completedCount = completedCountCursor.getInt(0); }
        finally { completedCountCursor.close(); }
        if (completedCount > 0) {
            final int completedTotal = completedCount;
            addButton(top, dangerBtn("Delete completed tasks (" + completedTotal + ")"), v ->
                    confirm("Delete completed tasks", "Delete all " + completedTotal + " completed tasks?", () -> {
                        db.getWritableDatabase().delete("todos", "done=1", null);
                        showAllTasks();
                    }));
        }
        content.addView(top);
        ...
        Cursor tasks = db.getReadableDatabase().rawQuery(
                "SELECT id,title,done,due_jdate,responsible,details FROM todos " +
                        "ORDER BY done ASC, CASE WHEN due_jdate IS NULL OR due_jdate='' THEN 1 ELSE 0 END, due_jdate ASC, created_at DESC, id DESC",
                null
        );
        try {
            if (!tasks.moveToFirst()) {
                ...
                do {
                    final long taskId = tasks.getLong(0);
                    final int taskDone = tasks.getInt(2);
                    LinearLayout item = card();
                    item.addView(tv((taskDone == 1 ? "✓ " : "") + tasks.getString(1), 16, textColor, Typeface.BOLD));
                    String due = tasks.getString(3) == null ? "" : tasks.getString(3);
                    String responsible = tasks.getString(4) == null ? "" : tasks.getString(4);
                    item.addView(tv((due.isEmpty() ? "No due date" : "Due: " + due) + (responsible.isEmpty() ? "" : " • Responsible: " + responsible), 12, mutedColor, Typeface.NORMAL));
                    String details = tasks.getString(5) == null ? "" : tasks.getString(5);
                    if (!details.isEmpty()) item.addView(tv(details, 13, textColor, Typeface.NORMAL));
                    addButton(item, outlineBtn("Edit"), v -> editTask(taskId));
                    addButton(item, outlineBtn(taskDone == 1 ? "Mark not done" : "Mark done"), v -> {
                        ContentValues cv = new ContentValues();
                        cv.put("done", taskDone == 1 ? 0 : 1);
                        update("todos", cv, taskId);
                        showAllTasks();
                    });
                    if (taskDone == 1) {
                        addButton(item, dangerBtn("Delete completed task"), v ->
                                confirm("Delete task", "Delete this completed task?", () -> {
                                    delete("todos", taskId);
                                    showAllTasks();
                                }));
                    }
