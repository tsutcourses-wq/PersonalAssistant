...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "3.8.1 Android";
    ...
    private void showDashboard() {
        ...
        notes.addView(tv("Notes / یادداشت", 18, textColor, Typeface.BOLD));
        notes.addView(tv("Write notes in English, فارسی, or both.", 12, mutedColor, Typeface.NORMAL));
        final EditText noteEdit = input("Note / یادداشت", getSetting("dashboard_note", ""), true);
        noteEdit.setMinLines(5);
        notes.addView(noteEdit);
        addButton(notes, btn("Save note / ذخیره یادداشت"), v -> {
            setSetting("dashboard_note", noteEdit.getText().toString());
            toast("Note saved.");
        });
    ...
    private LinearLayout buildDashboardCalendar() {
        ...
        addButton(modes, outlineBtn("Week"), v -> setDashboardCalendarMode("week"));
        addButton(modes, outlineBtn("Month"), v -> setDashboardCalendarMode("month"));
        addButton(modes, outlineBtn("Year"), v -> setDashboardCalendarMode("year"));
        box.addView(modes);

        LinearLayout nav = hbox();
        addButton(nav, outlineBtn("‹ Previous"), v -> shiftDashboardCalendar(-1));
        addButton(nav, outlineBtn("Today"), v -> resetDashboardCalendarToToday());
        addButton(nav, outlineBtn("Next ›"), v -> shiftDashboardCalendar(1));
        box.addView(nav);
    ...
    private void addDashboardWeekView(LinearLayout box) {
        ...
        box.addView(tv("Week containing " + anchor, 13, primaryColor, Typeface.BOLD));
