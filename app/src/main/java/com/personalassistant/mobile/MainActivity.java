...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "3.8.0 Android";
    ...
    private void showSettings() {
        ...
        server.addView(tv("Central server — Phase 1", 18, textColor, Typeface.BOLD));
        CentralServerClient.Settings serverSettings = loadCentralServerSettings();
        String serverAddress = serverSettings.url == null || serverSettings.url.trim().isEmpty() ? "Not configured" : serverSettings.url.trim();
        server.addView(tv("Server: " + serverAddress, 13, textColor, Typeface.NORMAL));
        server.addView(tv("Last downloaded revision: " + serverSettings.revision, 13, textColor, Typeface.NORMAL));
        server.addView(tv("This server-ready phase transfers a protected snapshot containing the database and managed files. Revision conflicts are detected. Use one device at a time until record-level synchronization is introduced.", 12, mutedColor, Typeface.NORMAL));
        addButton(server, btn("Configure central server"), v -> showCentralServerConfigDialog());
        addButton(server, outlineBtn("Test server connection"), v -> testCentralServer());
        addButton(server, outlineBtn("Upload all information + files"), v -> uploadCentralSnapshot(false));
        addButton(server, outlineBtn("Download all information + files"), v -> confirmDownloadCentralSnapshot());
