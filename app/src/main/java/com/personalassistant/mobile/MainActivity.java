...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "3.7.6 Android";
    ...
    private static final class ScreenSnapshot {
        final View root;
        final String title;
        ScreenSnapshot(View root, String title) {
            this.root = root;
            this.title = title;
        }
    }
    ...
    private void goBackOneStep() {
        if (!screenHistory.isEmpty()) {
            ...
            currentRootView = snapshot.root;
            currentScreenTitle = snapshot.title;
            setContentView(snapshot.root);
            View clock = snapshot.root.findViewWithTag("pa_clock");
            currentClockView = clock instanceof TextView ? (TextView) clock : null;
            applyFullscreenMode();
            return;
        }
    ...
    private void setScreen(String title, LinearLayout content) {
        boolean sameScreen = title.equals(currentScreenTitle);
        boolean canGoBack = currentRootView != null || !screenHistory.isEmpty() || !"Dashboard".equals(title);
        ...
        if (canGoBack && !"Dashboard".equals(title)) {
            ...
            back.setBackground(rounded(0x22FFFFFF, 14));
            back.setOnClickListener(v -> goBackOneStep());
            navigationRow.addView(back, new LinearLayout.LayoutParams(dp(52), dp(52)));
        }
        Button menu = btn("☰");
        menu.setTextSize(22);
        menu.setContentDescription("Open menu");
        menu.setMinWidth(dp(52));
        menu.setBackground(rounded(0x22FFFFFF, 14));
        ...
        if (currentRootView != null && !sameScreen && !skipHistoryOnce) {
            screenHistory.push(new ScreenSnapshot(currentRootView, currentScreenTitle));
        }
    ...
    private EditText dateField(LinearLayout parent, String label, String value, boolean yearOnly) {
        ...
        Button pick = outlineBtn(yearOnly ? "Select year" : "Shamsi date");
    ...
    private EditText timeField(LinearLayout parent, String label, String value) {
        ...
        View.OnClickListener listener = v -> {
            ...
            new TimePickerDialog(this, (view, h, m) -> edit.setText(String.format(Locale.US, "%02d:%02d", h, m)), hour, minute, true).show();
        };
    ...
    private void showJalaliPicker(EditText target, boolean yearOnly) {
        ...
        box.addView(tv("Shamsi year", 14, textColor, Typeface.BOLD));
        ...
        new AlertDialog.Builder(this)
                .setTitle(yearOnly ? "Select Shamsi year" : "Select Shamsi date")
                .setView(box)
    ...
    private void showRemindersForDate(String filterDate) {
        ...
        addButton(top, outlineBtn("Excel report by Shamsi month"), v -> showReminderReportMonths());
    ...
    private void editReminder(long id, String returnFilterDate) {
        ...
        addButton(form, outlineBtn("Back"), v -> {
            skipHistoryOnce = true;
            if (returnFilterDate == null || returnFilterDate.isEmpty()) showReminders();
            else showRemindersForDate(returnFilterDate);
        });
    ...
    private void showReminderReportMonths() {
        ...
        EditText yearEdit = input("Shamsi year", String.valueOf(Jalali.currentYear()), false);
        ...
        new AlertDialog.Builder(this)
                .setTitle("Select one or more Shamsi months")
                .setView(box)
                .setPositiveButton("Create Excel", (dialog, which) -> {
                    ...
                    catch (Exception ex) { toast("Enter a valid Shamsi year."); return; }
                    try {
    ...
    private void prepareReminderXlsx(int year, List<Integer> months) throws Exception {
        ...
        rows.add(new String[]{"Title", "Details", "Shamsi date", "Time"});
    ...
    private void editStudent(long id, String requestedLevel) {
        ...
        final EditText registrationYear = dateField(form, "Registration year (Shamsi)", registrationYearOnly(data.get("registration_date")), true);
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showStudents(); });
        content.addView(form);
        setScreen(exists ? "Student information" : "Add student", content);
    }
    ...
    private void showStudentTasks(long studentId) {
        ...
        addButton(top, outlineBtn("Back to student"), v -> { skipHistoryOnce = true; editStudent(studentId, ""); });
        content.addView(top);
    ...
    private void showStudentDocuments(long studentId) {
        ...
        setScreen("Student documents", content);
    }
    ...
    private void showAddGeneralTask() {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showAllTasks(); });
        content.addView(form);
        setScreen("Add task", content);
    }
    ...
    private void chooseProjectForTask() {
        ...
        new AlertDialog.Builder(this).setTitle("Select project").setItems(names.toArray(new String[0]), (d, which) -> showProjectTasks(ids.get(which))).show();
    }
    ...
    private void chooseStudentForTask() {
        ...
        new AlertDialog.Builder(this).setTitle("Select student").setItems(names.toArray(new String[0]), (d, which) -> showStudentTasks(ids.get(which))).show();
    }
    ...
    private void editProject(long id) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showProjects(); });
        content.addView(form);
    ...
    private void showProjectTasks(long projectId) {
        ...
        addButton(top, outlineBtn("Back to project"), v -> { skipHistoryOnce = true; editProject(projectId); });
        content.addView(top);
    ...
    private void showProjectStaff(long projectId) {
        ...
        setScreen("Project staff", content);
    }
    ...
    private void editProjectStaff(long projectId, long staffId) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showProjectStaff(projectId); });
        content.addView(form);
        setScreen(exists ? "Edit staff member" : "Add staff member", content);
    }
    ...
    private void showProjectFiles(long projectId, String section) {
        ...
        setScreen("Project " + section, content);
    }
    ...
    private void editWbs(long projectId, long itemId) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showWbs(projectId); });
        content.addView(form);
        setScreen(itemId > 0 ? "Edit WBS item" : "Add WBS item", content);
    }
    ...
    private void editCbs(long projectId, long itemId) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showCbs(projectId); });
        content.addView(form);
        setScreen(itemId > 0 ? "Edit CBS item" : "Add CBS item", content);
    }
    ...
    private void showSCurve(long projectId) {
        ...
        setScreen("S-curve data", content);
    }
    ...
    private void editSCurvePoint(long projectId, long pointId) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showSCurve(projectId); });
        content.addView(form);
        setScreen(pointId > 0 ? "Edit S-curve point" : "Add S-curve point", content);
    }
    ...
    private void editGantt(long projectId, long taskId) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showGantt(projectId); });
        content.addView(form);
        setScreen(taskId > 0 ? "Edit Gantt task" : "Add Gantt task", content);
    }
    ...
    private void editCourse(long id) {
        ...
        addButton(form, outlineBtn("Back"), v -> { skipHistoryOnce = true; showCourses(); });
        content.addView(form);
        setScreen(exists ? "Course page" : "Add course", content);
    }
    ...
    private void showCourseFiles(long courseId) {
        ...
        addButton(top, outlineBtn("Back to course"), v -> { skipHistoryOnce = true; editCourse(courseId); });
        content.addView(top);
        ...
        setScreen("Course documents", content);
    }
    ...
    private void showFileInventory() {
        ...
        setScreen("File Inventory", content);
    }
